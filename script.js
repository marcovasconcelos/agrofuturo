...
function nextSplashSlide() {
    if (splashSlides.length) {
        splashSlides[splashSlideIndex].classList.remove('active');
        splashSlideIndex = (splashSlideIndex + 1) % splashSlides.length;
        splashSlides[splashSlideIndex].classList.add('active');
    }
}
...
function toggleMobileMenu() {
    ...
    tabs.classList.toggle('mobile-show', mobileMenuOpen);
}
...
document.addEventListener('click', function(event) {
    ...
    const tabs = document.getElementById('tabsContainer');
    if (mobileMenuOpen && !tabs.contains(event.target) && !menuBtn.contains(event.target)) {
        toggleMobileMenu();
    }
});
...
function simular(cat, val, btn) {
    ...
    if (simState.energia === 'limpa') {
        lucro += 20000;
        if (impacto === "Alto") impacto = "Médio";
        else impacto = "Baixo";
    } else {
        lucro -= 10000;
        impacto = "Alto";
    ...
    if (simState.plantio === 'sustentavel') {
        lucro += 25000;
        if (impacto === "Alto") impacto = "Médio";
        else impacto = "Baixo";
        agua -= 12;
    ...
    let estrelas = "";
    if (impacto === "Baixo") estrelas = "★★★★★";
    else if (impacto === "Médio") estrelas = "★★★☆☆";
    else estrelas = "★☆☆☆☆";
...
// Inicializar simulador com valores padrão
setTimeout(() => {
    const btnsAgua = document.querySelectorAll('.opt-group:first-child .opt-btn');
    const btnsEnergia = document.querySelectorAll('.opt-group:nth-child(2) .opt-btn');
    const btnsPlantio = document.querySelectorAll('.opt-group:last-child .opt-btn');
    if (btnsAgua[0]) btnsAgua[0].classList.add('active');
    if (btnsEnergia[0]) btnsEnergia[0].classList.add('active');
    if (btnsPlantio[0]) btnsPlantio[0].classList.add('active');
}, 100);

// ================================================
// QUIZ (5 PERGUNTAS COM DICAS)
// ================================================

const perguntas = [
    {
        texto: "❓ Qual tecnologia utiliza sensores para irrigação precisa e reduz desperdício de água em até 60%?",
        opts: ["Drone Agrícola", "Irrigação Inteligente", "Trator Autônomo", "Biotecnologia"],
        correta: 1,
        dica: "💡 DICA: Pense em um sistema que entrega água direto na raiz da planta, gota a gota!"
    },
    {
        texto: "❓ Qual prática agrícola ajuda a recuperar solos degradados e sequestrar carbono da atmosfera?",
        opts: ["Monocultura", "Agricultura Regenerativa", "Queimada controlada", "Uso excessivo de fertilizantes"],
        correta: 1,
        dica: "💡 DICA: É uma prática que devolve vida ao solo, usando técnicas como plantio direto e adubação verde!"
    },
    {
        texto: "❓ Qual fonte de energia renovável é mais vantajosa para propriedades rurais remotas?",
        opts: ["Carvão Mineral", "Energia Solar", "Diesel", "Gás Natural"],
        correta: 1,
        dica: "💡 DICA: Vem do sol e não precisa de fios elétricos até a propriedade!"
    },
    {
        texto: "❓ O que são bioinsumos na agricultura sustentável?",
        opts: ["Produtos químicos tóxicos", "Defensivos naturais à base de organismos vivos", "Agrotóxicos sintéticos", "Fertilizantes industriais"],
        correta: 1,
        dica: "💡 DICA: São produtos feitos com fungos, bactérias ou insetos benéficos que combatem pragas naturalmente!"
    },
    {
        texto: "❓ Qual o benefício da rotação de culturas?",
        opts: ["Empobrece o solo", "Aumenta pragas", "Melhora a fertilidade do solo", "Reduz biodiversidade"],
        correta: 2,
        dica: "💡 DICA: Plantar diferentes culturas em sequência evita que o solo perca nutrientes específicos!"
    }
];
    ...
    q.opts.forEach((opt, idx) => {
        ...
        btn.onclick = () => {
            if (respostasUsuario[perguntaAtual] !== undefined) {
                document.getElementById('quizFeedback').innerHTML = '<span style="color:#c44536;">⚠️ Você já respondeu!</span>';
                return;
            }
            ...
            if (idx === q.correta) {
                document.getElementById('quizFeedback').innerHTML = '<span style="color:#2d8c5a;">✅ Correto! Muito bem!</span>';
            } else {
                document.getElementById('quizFeedback').innerHTML = `<span style="color:#c44536;">❌ Errado! ${q.dica}</span>`;
            }
...
function proximaPergunta() {
    ...
    if (respostasUsuario[perguntaAtual] === undefined) {
        document.getElementById('quizFeedback').innerHTML = '<span style="color:#c44536;">⚠️ Responda primeiro!</span>';
        return;
    }
...
function finalizarQuiz() {
    ...
    for (let i = 0; i < perguntas.length; i++) {
        if (respostasUsuario[i] === perguntas[i].correta) acertos++;
    }
    ...
    if (porcentagem >= 80) {
        classificacao = "🌿 EXPERT EM SUSTENTABILIDADE! 🏆";
    } else if (porcentagem >= 50) {
        classificacao = "🌱 BOM TRABALHO! Continue aprendendo! 💚";
    } else {
        classificacao = "📚 Que tal estudar mais com o Agribot? Vamos melhorar! 🌍";
    }
...
function reiniciarQuiz() {
    ...
    document.getElementById('quizResultadoArea').style.display = 'none';
...
function responderBot(pergunta) {
    ...
    // Dicas específicas por pergunta
...
// Adicionar evento de clique no easter egg
document.addEventListener('DOMContentLoaded', function() {
    const egg = document.getElementById('easterEgg');
    if (egg) {
        egg.addEventListener('click', ativarEasterEgg);
    }
});
